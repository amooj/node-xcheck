...
ArrayTemplate.parse = function (array, context){
  context = context || [];
  let itemTemplate = ValueTemplate.parse(array[0], context);
  if (itemTemplate.defaultValue !== undefined){
    throw new SyntaxError(context.join('.') + ': array item should not has a default value.');
  }
...
/**
 * Parses a object template definition.
 *
...
PropertyTemplate.prototype.validate = function (value, options) {
  ...
  if (value === undefined){
    if (this.required || !this.nullable){
      throw new TypeError('property \'' + context.join('.') + '\' is required but not found.');
    }
    if (options && options.applyDefaults){
      return this.template.defaultValue;
    }
  }
  else if (value === null){
    if (!this.nullable){
      throw new TypeError('property \'' + context.join('.') + '\' can not be null.');
    }
    return null;
  }
