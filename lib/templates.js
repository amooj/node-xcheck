 ...
 *   @property {Object} [udt] - user defined types
...
ValueTemplate.parse = function (value, options){
  ...
  if (tokens.next()){
    ...
    if (!traits.typeTest(defaultValue, typeName)){
  ...
  // lookup built-in and user defined types for the name
  let typeTemplate = exports.builtin[typeName];
  if (typeTemplate === undefined && options.udt){
    typeTemplate = options.udt[typeName];
  }
  if (typeTemplate === undefined){
    throw new SyntaxError(context.join('.') + ': unresolved type name \'' + typeName + '\'.')
  }
  return new typeTemplate(defaultValue);
};
...
PropertyTemplate.parse = function (nameSpec, valueSpec, options){
  ...
  if (defaultValue !== undefined){
    ...
    if (!traits.typeTest(defaultValue, template.type)){
